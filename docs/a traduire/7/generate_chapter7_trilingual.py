...
EN = LangPack(
    ...
    intro=(
        ...
        f"structural competitive advantage (US/EU CACI Power Mode of {fmt_en(us_eu_caci, 2)}:1 on the April 2026 snapshot, "
        f"operational installed compute raw ratio of {fmt_en(us_eu_raw, 1)}:1), accelerated by the 2026 Trump tariffs "
        f"and the concentration of compute in the United States ({fmt_en(us_share, 1)}% of global operational AI compute, "
    ...
    sections=[
        ...
        ("7.1.1 Short term (2026-2027): accelerating AI Factories", [
            f"The starting point is the infrastructure gap. The EU has approximately 35 GW of IT data center capacity compared to 53.7 GW in the United States and 19.6 GW in China. In strict AI compute, the April 2026 public dashboard establishes an EU(13) F_total share of 3.3% versus {fmt_en(us_share, 1)}% for the United States, representing a raw operational ratio of {fmt_en(us_eu_raw, 1)}:1 and a CACI Power Mode ratio (geometric formula F^0.40 x L^0.20 x R^0.15 / E^0.25) of {fmt_en(us_eu_caci, 2)}:1. Three immediate measures are required.",
            ...
            f"A key nuance from the Phys/Sov analysis of Chapter I (Fig 1.8): on physically installed compute, the EU is already largely sovereign ({fmt_en(eu_sov, 1)}% of EU F_total is held by EU operators). The window of vulnerability is therefore not so much on installed F as on the cloud workload layer (the compute actually used by EU companies, mostly hosted on AWS/Azure/GCP). The following recommendations primarily target this operational layer.",
        ]),
        ...
        ("7.7 Success conditions and limits", [
            ...
            f"Condition 3: European coherence. Intra-European fragmentation (27 energy regimes, divergent positions on nuclear, competing national sovereignty approaches) remains the main obstacle. Scenario C of Chapter V (asymmetric partnership, baseline {fmt_en(us_eu_caci, 2)}:1 toward 2.0-2.5:1) only works for Europe if it speaks with one voice in negotiations with Washington.",
            f"Condition 4: the time factor. The tipping point identified in Chapter V (2028, EU compute plus energy saturation, and potential activation of Cloud Sovereignty Mandates) imposes a constrained schedule. If the AI Factories are not operational and the EDF sites not connected by that date, the compute gap will solidify into structural dependence. The window for strategic action is between 2026 and 2028—after which positions crystallize around the {fmt_en(us_eu_raw, 1)}:1 raw / {fmt_en(us_eu_caci, 2)}:1 CACI Power Mode baseline.",
        ]),
    ],
    tables=[
        ("Table 23. Temporal matrix of strategic recommendations by axis (2026-2032).",
         f"Source: Author's construction; April 2026 baseline (US/EU operational raw compute {fmt_en(us_eu_raw, 1)}:1, CACI Power Mode {fmt_en(us_eu_caci, 2)}:1).",
         [
...
FR = LangPack(
    ...
    intro=(
        ...
        f"competitif structurel mesurable (CACI Power Mode US/UE de {fmt_fr(us_eu_caci, 2)}:1 sur le snapshot avril 2026, "
        f"ratio brut compute installe operationnel de {fmt_fr(us_eu_raw, 1)}:1), accelere par les tarifs Trump de 2026 "
        f"et la concentration du compute aux Etats-Unis ({fmt_fr(us_share, 1)} pct du compute IA operationnel mondial, "
    ...
    sections=[
        ...
        ("7.1.1 Court terme (2026-2027) : accelerer les AI Factories", [
            f"Le point de depart est l'ecart d'infrastructure. L'UE dispose d'environ 35 GW de capacite IT data centers contre 53,7 GW aux Etats-Unis et 19,6 GW en Chine. En compute IA strict, le tableau de bord public d'avril 2026 etablit une part UE(13) F_total de 3,3 pct contre {fmt_fr(us_share, 1)} pct pour les Etats-Unis, soit un ratio brut operationnel de {fmt_fr(us_eu_raw, 1)}:1 et un ratio CACI Power Mode (formule geometrique F^0,40 x L^0,20 x R^0,15 / E^0,25) de {fmt_fr(us_eu_caci, 2)}:1. Trois mesures immediates s'imposent.",
        ...
        ("7.7 Conditions de succes et limites", [
            ...
            f"Condition 3 : l'coherence europeenne. La fragmentation intra-europeenne (27 regimes energetiques, positions divergentes sur le nucleaire, approches nationales de souverainete concurrentes) reste le principal obstacle. Le scenario C du chapitre V (partenariat asymetrique, baseline {fmt_fr(us_eu_caci, 2)}:1 vers 2,0-2,5:1) ne fonctionne pour l'Europe que si elle parle d'une seule voix dans les negociations avec Washington.",
            f"Condition 4 : le facteur temps. Le point de basculement identifie au chapitre V (2028, saturation compute plus energie UE, et activation potentielle des Cloud Sovereignty Mandates) impose un calendrier contraint. Si les AI Factories ne sont pas operationnelles et les sites EDF non raccordes a cette date, le gap de compute se solidifiera en dependance structurelle. La fenetre d'action strategique se situe entre 2026 et 2028 - apres quoi les positions se cristallisent autour de la baseline {fmt_fr(us_eu_raw, 1)}:1 brut / {fmt_fr(us_eu_caci, 2)}:1 CACI Power Mode.",
        ]),
    ...
    tables=[
        ("Tableau 23. Matrice temporelle des recommandations strategiques par axe (2026-2032).",
         f"Source : construction de l'auteur ; baseline avril 2026 (compute brut operationnel US/UE {fmt_fr(us_eu_raw, 1)}:1, CACI Power Mode {fmt_fr(us_eu_caci, 2)}:1).",
         [
...
BR = LangPack(
    ...
    intro=(
        ...
        f"competitiva estrutural mensuravel (CACI Power Mode EUA/UE de {fmt_fr(us_eu_caci, 2)}:1 no snapshot de abril de 2026, "
        f"razao bruta de computacao instalada operacional de {fmt_fr(us_eu_raw, 1)}:1), acelerada pelas tarifas Trump de 2026 "
        f"e pela concentracao de computacao nos Estados Unidos ({fmt_fr(us_share, 1)}% da computacao de IA operacional global, "
    ...
    sections=[
        ...
        ("7.1.1 Curto prazo (2026-2027): acelerando as AI Factories", [
            f"O ponto de partida e a lacuna de infraestrutura. A UE possui aproximadamente 35 GW de capacidade de data center de TI em comparacao com 53,7 GW nos Estados Unidos e 19,6 GW na China. Em computacao de IA estrita, o painel publico de abril de 2026 estabelece uma participacao da UE(13) F_total de 3,3% contra {fmt_fr(us_share, 1)}% para os Estados Unidos, representando uma razao operacional bruta de {fmt_fr(us_eu_raw, 1)}:1 e uma razao CACI Power Mode (formula geometrica F^0,40 x L^0,20 x R^0,15 / E^0,25) de {fmt_fr(us_eu_caci, 2)}:1. Tres medidas imediatas sao necessarias.",
        ...
        ("7.7 Condicoes de sucesso e limites", [
            ...
            f"Condicao 3: coerencia europeia. A fragmentacao intra-europeia (27 regimes energeticos, posicoes divergentes sobre energia nuclear, abordagens nacionais de soberania concorrentes) continua sendo o principal obstaculo. O Cenario C do Capitulo V (parceria assimetrica, linha de base de {fmt_fr(us_eu_caci, 2)}:1 para 2,0-2,5:1) so funciona para a Europa se ela falar com uma unica voz nas negociacoes com Washington.",
            f"Condicao 4: o fator tempo. O ponto de virada identificado no Capitulo V (2028, saturacao de computacao e energia da UE e potencial ativacao dos Mandatos de Soberania de Nuvem) impoe um cronograma restrito. Se as AI Factories nao estiverem operacionais e os locais da EDF nao estiverem conectados ate essa data, a lacuna de computacao se solidificara em dependencia estrutural. A janela para acao estrategica e entre 2026 e 2028 - apos o que as posicoes se cristalizam em torno da linha de base de {fmt_fr(us_eu_raw, 1)}:1 bruto / {fmt_fr(us_eu_caci, 2)}:1 CACI Power Mode.",
        ]),
    ...
    tables=[
        ("Tabela 23. Matriz temporal de recomendacoes estrategicas por eixo (2026-2032).",
         f"Fonte: Construcao do autor; linha de base de abril de 2026 (razao bruta operacional de computacao EUA/UE de {fmt_fr(us_eu_raw, 1)}:1, CACI Power Mode de {fmt_fr(us_eu_caci, 2)}:1).",
         [
