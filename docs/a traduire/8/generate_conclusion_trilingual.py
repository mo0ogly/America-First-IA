...
EN = LangPack(
    ...
    sections=[
        ...
        ("2.1 A measurable and growing American competitive advantage", [
            f"The CACI (Compute-Adjusted Competitive Index) developed in this study allows for the quantification of structural asymmetry. On the April 2026 public dashboard snapshot, the US/EU(13) operational installed compute raw ratio stands at {fmt_en(us_eu_raw, 1)}:1, translated by a weighted geometric formula F^0.40 x L^0.20 x R^0.15 / E^0.25 into a CACI Power Mode ratio of {fmt_en(us_eu_caci, 2)}:1. The concentration of {fmt_en(us_share, 1)} percent of global operational AI compute in the United States (49.9 percent including planned capacities), an annual hyperscaler capex of 660-690 billion USD (higher than Sweden's GDP), and a frontier model training cost 5 to 10 times lower than the European cost confirm this dominance. The advantage is self-reinforcing: companies with abundant access to compute capture innovation and data rents that are very difficult to catch up with later (Chapter IV).",
            f"An important methodological nuance from Chapter I (Fig 1.8): the Phys/Sov decomposition rigorously calculated from the Epoch AI 'Owner' field reveals that asymmetry is differentiated by jurisdiction. The United States and China are fully sovereign over their installed compute (CACI Phys = CACI Sov). The EU is also very largely sovereign over its installed F ({fmt_en(eu_sov, 1)} percent EU-owned). The extreme case is that of the UAE: 99.6 percent of Emirati F_total is held by US-side operators (Stargate UAE, Microsoft, OpenAI), causing the sovereign CACI to drop from 55.7 (Physical) to 6.0. This Phys/Sov dissociation, formalized in Chapter V section 5.9.2 under the hypothesis of the 2028 Cloud Sovereignty Mandates activation, radically transforms the reading of asymmetry: European vulnerability lies not on installed compute but on the operational layer of cloud workloads (mostly hosted on AWS/Azure/GCP).",
        ]),
    ...
    tables=[
        ("Table 24. Summary of regional consequences of American AI protectionism.",
         f"Source: Author's construction, calibrated on the April 2026 snapshot (US {fmt_en(us_share, 1)}% operational AI compute, EU raw ratio {fmt_en(us_eu_raw, 1)}:1, CACI Power Mode {fmt_en(us_eu_caci, 2)}:1).",
         [
             ["Region", "Structural Position", "Main Impact", "Specific Risk"],
             ["Europe / France", "Tier 1, dependent on US GPU + cloud (72-80% workloads)",
              f"Compute gap {fmt_en(us_eu_raw, 1)}:1 raw / {fmt_en(us_eu_caci, 2)}:1 CACI; training costs x5-10",
...
FR = LangPack(
    ...
    tables=[
        ("Tableau 24. Synthese des consequences regionales du protectionnisme IA americain.",
         f"Source : construction de l'auteur, calibration sur le snapshot avril 2026 (US {fmt_fr(us_share, 1)} pct compute IA operationnel, ratio brut UE {fmt_fr(us_eu_raw, 1)}:1, CACI Power Mode {fmt_fr(us_eu_caci, 2)}:1).",
         [
             ...
             ["Europe / France", "Tier 1, dependante GPU + cloud US (72-80 pct workloads)",
              f"Compute gap {fmt_fr(us_eu_raw, 1)}:1 brut / {fmt_fr(us_eu_caci, 2)}:1 CACI ; couts training x5-10",
...
BR = LangPack(
    ...
    tables=[
        ("Tabela 24. Sintese das consequencias regionais do protecionismo de IA americano.",
         f"Fonte: Construcao do autor, calibrada no snapshot de abril de 2026 (EUA {fmt_fr(us_share, 1)}% da computacao de IA operacional, razao bruta da UE de {fmt_fr(us_eu_raw, 1)}:1, CACI Power Mode de {fmt_fr(us_eu_caci, 2)}:1).",
         [
             ...
             ["Europa / Franca", "Tier 1, dependente de GPUs + nuvem dos EUA (72-80% das cargas de trabalho)",
              f"Compute gap {fmt_fr(us_eu_raw, 1)}:1 bruto / {fmt_fr(us_eu_caci, 2)}:1 CACI; custos de treinamento x5-10",
