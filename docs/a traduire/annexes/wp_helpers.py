"""
Shared helpers for the 4 Chapter VI sub-chapter generators (FR).

Provides cover, header, section/table/notes/license rendering matching
the visual identity used in Chapters I-V (NAVY/GOLD/GREY palette,
Calibri typography, Light Grid Accent 1 tables).

Used by:
    chapitre6_france_europe_fr.py
    chapitre6bis_amerique_sud_bresil_fr.py
    chapitre6ter_asie_fr.py
    chapitre6quater_afrique_fr.py
"""
...
def add_cover(doc, *, chapter_label: str, chapter_subtitle: str):
    """Render the Chapter VI sub-chapter cover (FR)."""
    ...
    add_paragraph(doc,
                  f"Analyse geostrategique et economique integree - {chapter_label}",
                  align=WD_ALIGN_PARAGRAPH.CENTER,
...
def render_section(doc, title: str, paragraphs: list[str]):
    ...
    first = title.split()[0]
    # Sub-chapter sections use prefixes like "6.1", "6bis.1", "6ter.1.1", "6quat.2.4"
...
def render_table(doc, caption: str, source: str, rows: list[list[str]]):
...
def render_notes(doc, notes: list[str]):
...
def render_license(doc, page_footer: str):
